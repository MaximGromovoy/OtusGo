...
func (s *CurrencyServer) addCurrency(w http.ResponseWriter, r *http.Request) {
    ...
    if currencyType == "" {
        http.Error(w, "Тип валюты обязателен как параметр запроса", http.StatusBadRequest)
        return
    }

    // Проверяем поддерживается ли тип валюты
    if !currency.IsCurrencySupported(currencyType) {
        http.Error(w, fmt.Sprintf("Неподдерживаемый тип валюты: %s", currencyType), http.StatusBadRequest)
        return
    }
    ...
    if newCurrency == nil {
        http.Error(w, "Не удалось создать валюту", http.StatusInternalServerError)
        return
    }

    // Добавляем в репозиторий
    if err := s.repo.Add(newCurrency); err != nil {
        http.Error(w, fmt.Sprintf("Не удалось добавить валюту: %v", err), http.StatusInternalServerError)
        return
    }
...
func (s *CurrencyServer) handleCurrencies(w http.ResponseWriter, r *http.Request) {
    ...
    if requestedType != "" {
        if !currency.IsCurrencySupported(requestedType) {
            http.Error(w, fmt.Sprintf("Неподдерживаемый тип валюты: %s", requestedType), http.StatusBadRequest)
            return
        }
